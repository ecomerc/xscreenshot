...
namespace xscreenshot {
    class Program {
		...
		private static void TryLoadConfig(string file) {
			try {
				if (File.Exists(file) && (file.EndsWith(".conf") || file.EndsWith(".config"))) {
        ...
        static void Main(string[] args) {
            ...
            if (Directory.Exists(Config.Global.iOS.DevicesPath)) {
                ...
                foreach (Simulator device in devices) {
					...
					Process.Start (new ProcessStartInfo ("xcrun simctl erase #{aDeviceId}") {UseShellExecute = false});
                    ...
                    try {
						using (var app = Xamarin.UITest.ConfigureApp.iOS.EnableLocalScreenshots()
